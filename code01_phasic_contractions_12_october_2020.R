...
# This code takes skeletal muscle contraction data, in .csv format, to extract key performance indicators such as baseline tension, maximum twitch force, twitch duration, and first derivatives.
# This code was built to take data from the C4P (control-4-pulse) protocol for data collected on an Aurora Scientific Myograph.
# Protocol Parameters:
#   C4P - Data analysis
...
# 4. Normalized force = (measuredF - baseF)*9.80665/(Muscle Mass in Kgs)
...
dname = "~/Desktop/R_del_carlo_R_Sample_Code/C4P"

# Move working directory to data-containing directory, dname.
setwd(dname)
...
# Parse the directory using the forward slash.
x = strsplit(dname, "/")

# Concatenate components of dname to create a variable serving as a directory for the output.
...
hdrs <- c(
  ...
  "Genotype", # This is a duplicate column for ease of viewing.
  "Pulse", # This value, 1, 2, 3 or 4, describes the data collected from the 1st through 4th pulse recorded in a single file.
  ...
  "ContrAmpl", # This is the amplitude of the force produced after stimulus (i.e. the maximum force recorded within 1.5 seconds after the stimulus less the BaseF(N/g))
  "ToMaxF(ms)", # This is the time required to reach maximal force in milliseconds
...
for (file in files) {
  ...
  muscMass <- smm[which(smm$SnakeID == snake), muscle] / 1000 #this factor is proportional to denomination of muscle mass (mg vs g)
  sMusMassg <- muscMass
  ...
  stmRows <- as.integer(rownames(raw[raw$Stimulus > 1.0,])) # This cutoff of 1.0 can be tailored for specific protocols
  if (length(stmRows) < 4) {
  ...
  for (sr in stmRows) {
    ...
    rspF <- (raw$Force..g.[stmRows[1]:(stmRows[1] + 1499)] - meanF) * (0.00980665 / muscMass)
    oFline <- c(
      sSpecies,
      sGenotype,
      sMAMU,
    ...
    #  Use actual first-derivative regime based on a smoothed spline
    ...
    maxF <- max(rspF)
    t100p <- min(which(rspF >= maxF)) #defines the minimum time at which the force is less than or equal to the maximum
    # convert all times to ms from tenth of a ms
    t10p <- min(which(rspF > 0.1 * maxF)) #defines the point where 10% of the peak is achieved during contraction
    t50p <- t100p + errorless_min(which(rspF[t100p:length(rspF)] <= 0.5 * maxF)) #defines the point where 50% of the peak is achieved during relaxation post peak
    maxF1d <- max(rspF1d)
    minF1d <- min(rspF1d)
  ...
  graphics.off()
} #End of above for loops (3)

# Read the csv that was just made above
df = read.csv("./output/c4p1-force.csv")
# Write a file that will contain pulse-matched data extracted from each file
write.csv(t(df[order(df$Pulse, df$Snake, df$Muscle, df$Species),]), "./output/c4p1-force-rpt.csv")

# Subset the above dataframe for each of the four pulses per file and for each species.
df <- df[order(df$Pulse, df$Species, df$Genotype, df$Snake, df$Muscle), ]
...
# Create new reports to present the first derivatives by writing over object df
df = read.csv("./output/c4p1-force1d.csv")
write.csv(t(df[order(df$Pulse, df$Snake, df$Muscle),]), "./output/c4p1-force1d-rpt.csv")
df <- df[order(df$Pulse, df$Species, df$Genotype, df$Snake, df$Muscle), ]
for (p in unique(df$Pulse)) {
  for (s in unique(df$Species[which(df$Pulse == p)])) {
    write.csv(df[which(df$Pulse == p &
...
for (each in dataspecies) {
  ...
  boxplot(species_summary_subset$ContrAmpl~species_summary_subset$Genotype, xlab=label, ylab="Peak Force Ouput (Newtons / gram muscle)")
}
